...
def handle_build(dist: str = 'dist'):
    ...
    md5_wheel = hashlib.md5(open(wheel_info.file, 'rb').read()).hexdigest()
    md5_sdist = hashlib.md5(open(sdist_info.file, 'rb').read()).hexdigest()
    wheel_name, sdist_name = str(wheel_info.file), str(sdist_info.file)
